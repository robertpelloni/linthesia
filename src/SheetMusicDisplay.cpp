...
void SheetMusicDisplay::Draw(Renderer &renderer, int x, int y, const TranslatedNoteSet &notes, microseconds_t show_duration,
  ...
  renderer.SetColor(Renderer::ToColor(240, 240, 240, 255));
...
void SheetMusicDisplay::DrawStaffLines(Renderer &renderer, int x, int y, int width, int height) const {
  ...
  renderer.SetColor(Renderer::ToColor(0, 0, 0, 255));
  // Treble
  for (int i = 0; i < 5; ++i) {
      renderer.DrawQuad(x, topY + i * lineSpacing, width, 2);
  }

  // Bass
  for (int i = 0; i < 5; ++i) {
      renderer.DrawQuad(x, bottomY - i * lineSpacing, width, 2);
  }
}
...
void SheetMusicDisplay::DrawNotes(Renderer &renderer, int x, int y, int width, int height,
  ...
  for (const auto& note : notes) {
      if (note.start < current_time + show_duration && note.end > current_time) {
          ...
          renderer.SetColor(Track::ColorNoteWhite[track_properties[note.track_id].color]);
          renderer.DrawQuad(noteX, noteY, 12, 8); // placeholder note head
      }
  }
}
